...
- 控制器按**游戏空间**的 3 格预取提问，而速度倍率改的是游戏时钟（`accumulator += elapsed * speed`）。
  0.5× 时，覆盖这 3 格需要 **1000 ms 墙钟**，不是 500 ms。
- 于是「500 ms 截止」只在 1× 时成立；别的速度下「执行率」被系统性高估或低估。

落地结果：

1. **控制器不缩放预取。** 它仍按 `driver.prefetch` 这个固定的**游戏空间**距离提问，
   于是墙钟窗口恰好是 `budgetMs / speed` —— 0.5× 下模型确实有双倍时间，2× 下一半。
   ⚠️ 原 P2-3 实现成了 `driver.prefetch × speed`（意图是「永远尽早问」），
   结果窗口被钉成恒定值 `prefetch × T`，与下面第 2、3 条直接冲突。2026-09-25 已改回，
   取舍过程见 `plan-multi-game.md` §7 决策 ①。
