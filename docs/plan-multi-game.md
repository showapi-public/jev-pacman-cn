...
新增：`components/shell/{GameSwitcher,GameNavCard,NavHome}.tsx`、`lib/use-model-catalogue.ts`、
`app/page.tsx` 改为双分支（1 个游戏 → 重定向，≥2 个 → 导航首页，与设计规范 §2.5 一致）；
`AppShell` 增加 `nav` 槽（切换器按 §2.1 的内容顺序放在游戏名之后）。

四处计划没写、执行时定的：

1. **「选哪条模型」抽成 `useModelCatalogue`。** 导航首页要摆同一个控件（设计规范 §2.5），
   而它没有「一局」可言、拿不到 `useGameSession`。抽出来的真正收益是**键与那条
   「存的 id 已从目录消失」的规则只有一份** —— 写两遍的话，环境里删掉一条模型线时两处会
   表现不一致。**「换模型重开一局」不在这个 hook 里**：那是「一局」的性质，留在会话 hook。
2. **切换器的位置按 §2.1 落**（品牌 · 切换游戏 · 状态胶囊 · …），`AppShell` 为此开了个
   `nav` 槽。**§10 的键盘顺序清单原来写的是「主操作 → 切换游戏」，与 §2.1 矛盾** ——
   按 §2.1 实现并同步修正了 §10（切换器在游戏名之后、状态胶囊之前）。
3. **`GAME_META.length === 1` 时 `/` 仍重定向**（P2-6b 拍板的行为不变），≥2 个才出导航页。
   两个分支读同一张表、都不写死 id，加第三个游戏时这里零改动。
4. 导航首页**不复用**锁高度的 `.app-shell`（§2.5），`min-h-dvh` 流式壳。

**探针抓到并修掉的一个真问题**：`GameSwitcher` 把 `<ul>` 放进了 `DialogDescription` ——
Radix 把 Description 渲染成 `<p>`，**`<p>` 里不能嵌 `<ul>`**（React 在 hydration 时报
非法嵌套，dev 日志可见）。`SnakeHelp` 抄了同一模式、`PacmanHelp` 是**既有同病** ——
三处一起改成 `DialogDescription asChild` + `div`。

**探针修完后的实测口径（三条都属环境，不是回归）**：

- **dev 模式下 mock 的墙钟延迟经常超过 500ms 预算**（实测 `POST /api/decide` 400–2000ms，
  主要是 dev 服务器开销），于是兜底被频繁触发、「最新一条」常常是还没等到答案的提问 ——
  决策卡的「模型」格会显示「—」。判定改用导出数据：**有答案的记录 `model` 都非空**
  （65 条里 15 条是及时抵达的答案，其余是超预算的提问与兜底）。这不是本次改动引入的，
  `jev-game` 的 live 测试在 dev 模式下一直有 4–9 次兜底。
- 模型目录只有一条时「切模型」无从切换：本机用 `JEV_MODELS='default|||;alt|备用条目|||'`
  临时起了一个双条目目录来验（目录是每次请求重读的，不用改 `.env.local`）。

**实测**（`/tmp/p3-4.mjs`，CDP 代理，**30/30 通过**）：蛇机台 —— 一层滚动条（`overflowY=0`）、
画布不被裁（可见高度/自身高度 = 1）、六个读数齐、标题与 `--self` 来自蛇的 meta、
开局后步数上涨、预算 500ms、键盘顺序（跳转链接 → 切换游戏 → 主操作 → …）、
导出 JSON 含 `game`/`model`/非空 `decisions`、切模型即重开（历史清空、步数从 34 回到 2）、
新局继续有决策进历史；互切 —— 弹层切到吃豆人（琥珀 `--self`）再切回蛇（绿 `--self`），
**两台机台的历史互不串**（各自从空开始），「返回游戏导航首页」能到 `/`；
导航首页 —— 标题「游戏台」、两张卡可进且决策形态是文字、不放切换器、
页内没有自己的滚动容器、模型选择可用且写入 `jev:model` 共用键。

**留给 P4-1 的**：README 的过时处、设计规范 §10 逐条复跑、`docs/review.md` 与收尾报告、
skill 沉淀。
